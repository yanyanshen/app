...
class CoachController extends CommonController
{
    public function upcertificate($type,$userid,$city)
    {
        ...
        $_POST['cityid']=M("citys")->field("id")->where("cityname like '%$city%'")->cache(true,300)->find()['id'];
        $table=shenfen($type);
        M($table)->where("userid='$userid'")->save($_POST);
        if($type=='jl'){
            $trainid=$_POST['trainid'];
            if(M("coachtrain")->where("coachid='$userid'")->cache(true,300)->find()){
    ...
    public function getpublicList($start,$type,$latitude,$longitude)
    {
			...
			$cityid=M("citys")->field("id")->where("cityname like '%$address%'")->cache(true,300)->find()['id'];
			switch($type){
				case 'jx':
    ...
    public function getAllList($objectid,$start,$type)
    {
		if($type=='jl'){
			...
			if($count==0){
				$objectid=M("coach")->field("masterid")->where("userid='$objectid'")->cache(true,300)->find()['masterid'];
			}
		}
    ...
    {
        $m = M('consult');
        if ($m->where("objectid='$objectid'")->cache(true,300)->find()) {
    ...
    public function coachupload()
    {
        ...
        if (M('img')->addAll($mm) && M($table)->where("userid='$userid'")->setInc("piccount", $c)) {
            $img=M('img')->field("id,imgname")->where("userid='$userid'")->cache(true,300)->select();
            foreach($img as $k=>$v){
	...
	public function getTeacherDetail($objectid,$userid,$type){
        ...
        ->join("xueche1_evaluating e on e.masterid=u.userid and e.objecthingid='$objectid'")->order('e.id desc')->limit(2)->cache(true,300)->select();
        $p2 =M("evaluating")->field("id,masterid,time,masterid,content,score")->where("masterid='1' and objecthingid='$objectid'")->order('id desc')->cache(true,300)->select();
	...
	if($img>0){
        ...
        $trainclass=M("trainclass")->field("tcid,name,include,masterid,time,carname,mode,officialprice,whole517price,prepay517price,prepay517deposit,classtime")->where("masterid='$objectid'")->cache(true,300)->select();
        $mm['class'] = $trainclass;
	...
	public function addClass(){
		 try {
			if(empty($_POST)){
		     ...
		     $_POST['time']=date("Y-m-d H:i:s");
		     $_POST['tcid']=M("trainclass")->field("id")->order("id desc")->cache(true,300)->find()['id']+1;
            if(M("trainclass")->add($_POST)){
                $class=M("trainclass")->field("tcid,name,carname,include,picktype,traintype,officialprice,whole517price,prepay517price,jtype,prepay517deposit,classtime,objectsecondcount,linetime,objectthirdcount")->where("masterid='$masterid'")->order('id desc')->cache(true,300)->find();
                $info=result(0, $class);
                echo $info;
			}else{
				echo  type(1, "添加失败");
 ...
 public function returnallstu($masterid='A25BF70C-D5FC-2956-5760-06611F40EBC7',$start=0,$subjects=1,$type='jl'){
	  ...
	  $wheres="s.Cl_type='y'";
          if($type=='jx' || $type=='sjl'){
              ...
              $stu=M("student s")->field("u.img,u.userid,s.truename,s.masterid,s.note,s.phone,s.address,s.pickupaddress,s.trainclass,s.subjects,s.paytype")->join("xueche1_user u on s.userid=u.userid and s.masterid='$masterid' and s.subjects=$subjects and $wheres")->limit("$start,10")->cache(true,300)->select();
             
	 $masterid=M('coach')->field("userid")->where("$where='$masterid'")->cache(true,300)->select();
              foreach ($masterid as $k=>$v){
                  $cid=$v['userid'];
                  $stu1[$k]=M("student s")->field("u.img,u.userid,s.truename,s.masterid,s.note,s.phone,s.address,s.pickupaddress,s.trainclass,s.subjects,s.paytype")->join("xueche1_user u on s.userid=u.userid and s.masterid='$cid' and s.subjects=$subjects and s.coachid=1 and $wheres")->limit("$start,10")->cache(true,300)->select();
                  if(empty($stu1[$k])){
                      unset($stu1[$k]);
                  }else{
              ...
              $stu=M("student s")->field("u.img,u.userid,s.truename,s.note,s.masterid,s.phone,s.address,s.pickupaddress,s.trainclass,s.subjects,s.paytype")->join("xueche1_user u on s.userid=u.userid and s.masterid='$masterid' and s.subjects=$subjects and $wheres")->limit("$start,10")->cache(true,300)->select();
          }
          $info= result(0,$stu);
      } catch (Exception $e) {
          $info= type(1,'返回失败');
	...
	public function delCoachimg(){
	 try {
           ...
           $i=0;
           foreach ($_POST as $v){
               $url=M("img")->field("imgname")->where("id=$v")->cache(true,300)->find()['imgname'];
               unlink($url);
	...
	public function addStudent(){
		...
		if(issetAccount($account)){
	        ...
	        if(M('student')->where("account='$account'")->cache(true,300)->find()){
	            echo  type(3,'该学员已存在');
	            return;
	        }else{
	            $_POST['userid']=guid();
	            ...
	            if(M("student")->add($_POST)){
	                 ...
	                 M("user")->add($_POST);
	                 $stu=M("student")->field("userid,masterid,truename,phone,address,pickupaddress,trainclass,subjects,note,paytype")->where("account='$account'")->cache(true,300)->find();
                     $info= result(0,$stu);
	            }else{
	                echo  type(1,'添加失败');
	            }
    ...
    public function returnClass($userid,$type){
	    try {
	        if($type=='jl'){
	            $jltype=M("coach")->field("jltype,masterid,boss")->where("userid='$userid'")->cache(true,300)->find();
	            if($jltype['jltype']==0){
	        ...
	        $class=M("trainclass")->field("tcid,name,include,carname,picktype,traintype,officialprice,whole517price,prepay517price,jtype,prepay517deposit,classtime,linetime,objectsecondcount,objectthirdcount")->where("masterid='$userid'")->cache(true,300)->select();
	        $info=result(0, $class);
	    } catch (Exception $e) {
	        $info=type(1,'失败');
	    }
	...
	public function getAbstract($userid,$type){
	    try {
	        $table=shenfen($type);
	        $coachinfo=M($table)->field("introduction,timeflag,pickrange")->where("userid='$userid'")->cache(true,300)->find();
    ...
    public function getCoachimg(){
        try {
            $userid=I('post.userid');
            $coachinfo=M("img")->field("id,imgname")->where("userid='$userid'")->cache(true,300)->select();
    ...
    public function updateStu($objectid,$key,$value){
        try {
            if(M("student")->where("userid='$objectid'")->setField($key,$value)){
                $stu=M("student")->field("userid,masterid,truename,phone,address,pickupaddress,trainclass,subjects,note,paytype")->where("userid='$objectid'")->cache(true,300)->find();
                $info=result(0, $stu);
            }else{
                $info=type(1,'修改失败');
            }
   ...
   public function getCoachs(){
        try {
            ...
            $table=shenfen($type);
            //如果是指导员的话获取学车成本
            $class['class']=M("trainclass")->field("tcid,name,include,carname,picktype,traintype,officialprice,whole517price,prepay517price,jtype,prepay517deposit,classtime,linetime,objectsecondcount,objectthirdcount")->where("masterid='$userid'")->cache(true,300)->select();
            if($type=='jl'){
                $jltype=M("coach")->field("jltype,masterid")->where("userid='$userid'")->cache(true,300)->find();
                if($jltype['jltype']==0 || $jltype['jltype']==3){
                    $masterid=$jltype['masterid'];
                    $class['class']=M("trainclass")->field("tcid,name,include,carname,picktype,traintype,officialprice,whole517price,prepay517price,jtype,prepay517deposit,classtime,objectsecondcount,linetime,objectthirdcount")->where("masterid='$masterid'")->cache(true,300)->select();
                }
            }
            //获取简介
            $class['introduction']=M($table)->field("pickrange,introduction,timeflag")->where("userid='$userid'")->cache(true,300)->find();
            //获得环境
            $class['img']=M("img")->field("id,imgname")->where("userid='$userid'")->cache(true,300)->select();
    ...
    public function upcertificate1($type,$userid,$city)
    {
        ...
        if($type=='jl'){
            $trainid=$_POST['trainid'].',';
            if(M("coachtrains")->where("coachid='$userid'")->cache(true,300)->find()){
    ...
    public function teacherDetail($objectid='',$userid='1ACCA599_8206_F1DA_90D3_BCFC6243E415',$type='jl'){
        ...
        ->join("xueche1_evaluating e on e.masterid=u.userid and e.objecthingid='$objectid'")->order('e.id desc')->cache(true,300)->find();
        if(!$mm['message']){
            $mm['message'] = M("evaluating")->field("id,time,masterid,objectid,content,score,num,replycount,evaluate")
            ->where("objecthingid='$objectid'")->order('id desc')->cache(true,300)->find();
        }
        $mm['consultcount']=M('consult')->where("objectid='$objectid'")->count();
        $mm['consult'] = M('consult c')->field("c.id,c.time,c.masterid,c.objectid,c.content,c.replycount,u.nickname,u.img")
        ->join("xueche1_user u on u.userid=c.masterid and c.objectid='$objectid'")->order('id desc')->cache(true,300)->find();
        $img = M('img')->where("userid='$objectid'")->count();
        if($img>0){
            $imgname1 = M('img')->field("imgname ")
            ->where("userid='$objectid'")//先写死
        ...
        $trainclass=M("trainclass")->field("tcid,name,include,masterid,time,carname,mode,officialprice,whole517price,prepay517price,prepay517deposit,classtime")->where("masterid='$objectid'")->cache(true,300)->select();
        $mm['class'] = $trainclass;
        if(M('attention')->where("userid='$userid' and objectid='$objectid'")->cache(true,300)->find()){
            $mm['conflag']=1;
        }else{
            $mm['conflag']=0;
        }
	...
	public function conreplyList($objectid='A2194076_B223_BBEB_4B33_0D59C8CAD108',$start=0,$master='objectid')
    {
        ...
        ->order('id desc')->cache(true,300)->select();
        if($mm){
            foreach($mm as $k=>$v){
                $id=$mm[$k]['cid'];//那条咨询的id
                ...
                $mm[$k]['consult']=M('consult')->field("id as cid,time,masterid,objectid,content,lasttime,type")->where("id=$id")->cache(true,300)->find();
                if($master=='masterid'){
                    $flag='stuflag';
			 ...
			 M('replyconsult')->where("cid=$id")->setField('flag','y');
                ...
                $mm[$k]['consult']['nickname']=M($table)->field("nickname")->where("userid='$obj'")->cache(true,300)->find()['nickname'];
            }
        }else{
            $mm=[];
        }
        if($master=='objectid'){
		   $flag='flag';
            ...
            $mmm=M('consult c')->field("c.id as cid,c.time,c.masterid,c.objectid,c.content,c.lasttime,c.type,u.nickname,c.replycount")->join("xueche1_user u on u.userid=c.masterid and c.objectid='$objectid' and (c.replycount=0 or c.replycount=1)")->order("c.id desc")->limit("$start,10")->cache(true,300)->select();
	 ...
	 public function evalreplyList($objectid='22A0A301_8877_C0EE_CB6E_E4D24D46DA3A',$start=0,$master='objecthingid')
    {
        ...
        if($mm){
            foreach($mm as $k=>$v){
                ...
                $mm[$k]['evaluating']=M('evaluating')->field("id,time,masterid,objecthingid,content,lasttime,type,score,evaluate")->where("id=$id")->cache(true,300)->find();
                if($master=='masterid'){
                    $flag='stuflag';
		 ...
		 M('replyevaluate')->where("tid=$id")->setField('flag','y');
                ...
                $mm[$k]['evaluating']['nickname']=M($table)->field("nickname")->where("userid='$obj'")->cache(true,300)->find()['nickname'];
            }
        }else{
            $mm=[];
        }
       //如果是教练端
        if($master=='objecthingid'){
		 $flag='flag';
            ...
            $mmm=M('evaluating c')->field("c.id as tid,c.time,c.masterid,c.objecthingid,c.content,c.lasttime,c.type,u.nickname,c.score,c.evaluate")->join("xueche1_user u on u.userid=c.masterid  and c.objecthingid='$objectid' and (c.replycount=0 or c.replycount=1)")->order("c.id desc")->limit("$start,10")->cache(true,300)->select();
            $mm=array_merge($mmm,$mm);
        }
        $info = result(0, $mm);
        echo $info;
    ...
    public function createconsult($masterid='8CF687BE_A1AF_5F50_5C3F_C051BB0218A7',$objectid='22A0A301_8877_C0EE_CB6E_E4D24D46DA3A',$content='你好',$type='jx')
    {
        ...
        if (M('consult')->add($_POST)) {
            $cinfo = M("consult c")->field("c.id,c.time")->where("masterid='$masterid'")->order("c.id desc")->cache(true,300)->find();
    ...
    public function createreply($cid=2,$masterid="1ACCA599_8206_F1DA_90D3_BCFC6243E415",$objectid="F5DCBE66_B741_D0EA_91ED_3A137EE99519",$content='你好',$type='jl')
    {
        ...
        if (M('replyconsult')->add($_POST)){
            ...
            $result=M("replyconsult")->field("id,time")->where("masterid='$masterid' and objectid='$objectid'")->order("id desc")->cache(true,300)->find();
            if($type=='xy'){
    ...
    public function replyevaluate($tid=95,$masterid="22A0A301_8877_C0EE_CB6E_E4D24D46DA3A",$objectid="8CF687BE_A1AF_5F50_5C3F_C051BB0218A7",$content='你好',$type)
    {
        ...
        if (M('replyevaluate')->add($_POST)){
            ...
            $result=M("replyevaluate")->field("id,time")->where("masterid='$masterid' and objectid='$objectid'")->order("id desc")->cache(true,300)->find();
            if($type=='xy'){
    ...
    {
        $result=M('replyconsult')->field("id,time,masterid,objectid,content")->where("cid=$id")->cache(true,300)->select();
        $info = result(0, $result);
        echo $info;
    }
    //新的评论列表
    ...
    {
        $result=M('replyevaluate')->field("id,time,masterid,objectid,content")->where("tid=$id")->cache(true,300)->select();
        $info = result(0, $result);
        echo $info;
    }
	 //教练端咨询提问列表
