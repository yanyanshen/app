...
class TeaManage1Controller extends CommonController
{
   ...
   public function returnTrain($userid='D18CF959-EDB7-B2C2-C8D0-85CFB1415721'){
       try {
           $train=M('schooltrains')->field("trainid")->where("schoolid='$userid'")->cache(true,300)->find()['trainid'];
	   if($train){
		$train=explode(',',rtrim($train,','));
         	foreach ($train as $k=>$v){
               		$train[$k]=M('train')->field("id,trname")->where("id=$v")->cache(true,300)->find();
           	}
	   }else{
		$train=[];
	   }
   ...
   public function teaManage($type='jl',$userid='A2194076-B223-BBEB-4B33-0D59C8CAD108'){
	   ...
	   //$_POST['jltype']=2;
           ...
           $coach['timeflag']=M($table)->field("timeflag,jtype")->where("userid='$userid'")->cache(true,300)->find();
           //订单数
           ...
           if($type=='jl'){
               ...
               $land=M("coachlands")->field('landmarkid')->where("coachid='$userid'")->cache(true,300)->find()['landmarkid'];
               $coach['land']=substr_count($land,',');
               if($_POST['jltype']==2){
                   $coach['coachcount']=M("coach")->field("userid")->where("boss='$userid'")->count();
                   $coachs=M("coach")->field("userid")->where("boss='$userid'")->cache(true,300)->select();
                   //加上小打工的教练的学员人数和订单数
               ...
               //地标管理
               $land=M("schoollands")->field('landmarkid')->where("schoolid='$userid'")->cache(true,300)->find()['landmarkid'];
               $coach['land']=substr_count($land,',');
               //教练人数
               $coach['coachcount']=M("coach")->field("userid")->where("masterid='$userid'")->count();
               $coachs=M("coach")->field("userid")->where("masterid='$userid'")->cache(true,300)->select();
               //加上小打工的教练的学员人数和订单数
               foreach ($coachs as $k=>$v){
                   $uid=$v['userid'];
               ...
               $land=M("guiderlands")->field('landmarkid')->where("guiderid='$userid'")->cache(true,300)->find()['landmarkid'];
               $coach['land']=substr_count($land,',');
           }
           $info=result(0, $coach);
       } catch (Exception $e) {
   ...
   public function returnCoachinfo($userid='4F35E74E-38DE-EC47-A0C9-9E18303ACDBB'){
       try {
           $coach=M("coach")->field("userid,img,nickname,jtype,timeflag")->where("userid='$userid'")->cache(true,300)->find();
           //加上小打工的教练的学员人数和订单数
           //当前所教学员
           ...
           //所在基地
           $trainid=M("coachtrains")->field("trainid")->where("coachid='$userid'")->cache(true,300)->find()['trainid'];
	   if($trainid){
		$train=rtrim($trainid,',');
		$coach['train']=M('train')->field("trname")->where("id=$train")->cache(true,300)->find()['trname'];
	   }else{
	  	$coach['train']=null;
	   }
           $info=result(0, $coach);
   ...
   public function addCoach($phone){
       try {
           ...
           $c=M('coach')->field("$f1,userid")->where("account='$phone'")->cache(true,300)->find();
           if($c){
               $userid=$c['userid'];
               $t=M('coachtrains')->where("coachid='$userid'")->cache(true,300)->find();
               if($c['masterid']!=''){
   ...
   public function landManage($userid="D18CF959-EDB7-B2C2-C8D0-85CFB1415721",$type='jx',$city='上海'){
       try {
           $cityid=M("citys")->field("id")->where("cityname like '%$city%'")->cache(true,300)->find()['id'];
           if($type=='jx'){
               $table='schoollands';
               $field="schoolid";
           }elseif($type=='jl'){
           ...
           //找地标及名字
           $county=M("countys")->field("id,countyname")->where("masterid=$cityid")->cache(true,300)->select();
           $land=M($table)->field("landmarkid")->where("$field='$userid'")->cache(true,300)->find()['landmarkid'];
	   if($land){
		$land=explode(',',rtrim($land,',')) ;
               foreach ($county as $k=>$v){
                   foreach ($land as $kk=>$vv){
                       $c[$kk]=M('landmark')->field("id,landname,masterid")->where("id=$vv")->cache(true,300)->find();
   ...
   public function operLand($userid,$type,$landid){
       try {
           ...
           if(M($table)->where("$obj='$userid'")->cache(true,300)->find()){
   ...
   public function returnLand(){
       try {
           ...
           $land=M("landmark")->field("id,landname")->where("masterid=$countyid")->cache(true,300)->select();
           $info=result(0,$land);
       } catch (Exception $e) {
           $info=type(1, "删除失败");
       }
	 ...
	 public function trainManage($userid='5196C331_1DA3_E56C_DAC1_5EF8655FF96B'){
       try {
           ...
           if($train){
               $train=explode(',', rtrim($train,','));
               foreach($train as $k=>$v){
                   $train[$k]=M('train')->field("id,trname,address")->where("id=$v")->cache(true,300)->find();
...
//删除添加一个借口(只有驾校有)
  //删除添加一个借口(只有驾校有)
   public function operTrain($userid='5196C331_1DA3_E56C_DAC1_5EF8655FF96B',$trainid='1,79,'){
       try {
           $train=M("schooltrains")->field('id')->where("schoolid='$userid'")->cache(true,300)->find();
           if($train){
   ...
   public function returnTrainCoach($trainid=14,$userid='5196C331-1DA3-E56C-DAC1-5EF8655FF96B'){
       try {
           //教练列表
           $coach=M("coachtrains")->field("coachid")->where("schoolid='$userid' and locate($trainid,trainid)")->cache(true,300)->select();
           foreach($coach as $k=>$v){
               $uid=$v['coachid'];
               $coach[$k]['coach']=M("coach")->field("nickname,img")->where("userid='$uid'")->cache(true,300)->find();
   ...
   //给教练修改基地,传修改之后的所拥有的基地
       ...
       try {
           ...
           $tr=M("coachtrains")->field('id')->where("coachid='$objectid' and $master='$userid'")->cache(true,300)->find();
           if($tr){
   ...
   public function returnCoachStu($masterid='4ED02DF4-D6EF-150C-5046-08011F054349',$start=0,$type){
       try {
           ...
           $date=M("student s")->field("u.img,u.nickname,s.note,s.paytype")->join("xueche1_user u on s.userid=u.userid and s.$where='$masterid'")->limit("$start,10")->cache(true,300)->select();
           $info = result(0, $date);
       } catch (Exception $e) {
           $info=type(1, "返回失败");
       }
