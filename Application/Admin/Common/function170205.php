    ...
    function pd($account, $pass, $type, $agent)
{
	...
	if(M("user")->where($where)->find()){$shenfen='xy';$table='user';
	}else if(M("coach")->where($where)->find()){$shenfen='jl';$table='coach';$field.=",masterid,jltype";
	}else if(M("school")->where($where)->find()){$shenfen='jx';$table='school';
	}else if(M("guider")->where($where)->find()){$shenfen='zdy';$table='guider';
	}else{ $info = type(1, '账号不存在');  echo $info; return;	}
	if($shenfen==$type){
		$m=M($table)->field($field)->where($where)->find();
		if($pass==$m['pass']){
			unset($m['pass']);
			$u = M('conf');
      ...
      function type($code, $message, $data, $v = 'ios', $t)
    {
      ...
      function ver($v, $t)
    {
        $vv = M('version')->field("id,maxversion,minversion")->select();
        if ($v == 'ios') {
            if ($t == 'jl') {
                $max = $vv[1]['maxversion'];
            } else {
     ...
     */
      ...
      function trainprice($masterid,$jtype){
            $data=M("price")->field("id,weekdays,timebucket,price,jtype,subjects")->where("masterid='$masterid' and jtype='$jtype'")->select();
            return $data;
      }
    /**
     * 文件上传
	...
	*/
    ...
    {//96980ac7cf166499cbbcc946687fb414
	 ...
	 //注册时一次统计多张表看是否存在
      function issetAccount($account){
		$where="account='$account'";
		if(M("user")->where($where)->find()){$count=1;
		}else if(M("coach")->where($where)->find()){$count=1;
		}else if(M("school")->where($where)->find()){$count=1;
		}else if(M("guider")->where($where)->find()){$count=1;
		}else{$count=0;}
        return $count;
    }
      ...
      function getCoachList($start,$cityid,$latitude,$longitude)
    {
        ...
        $m = M("coach");
        if ($m->count() > 0) {
               $mm = $m->field($field)->where("cityid=$cityid and verify=3")->limit("$start,5")->select();
            foreach($mm as $k=>$v){
                $id=$v['userid'];
                $mas=$mm[$k]['masterid'];
	...
	function returnfield($type,$userid){
        ...
        switch($type){
            case 'jl':
                ...
                $user=M("coach")->field($field)->where("userid='$userid'")->find();
                $masterid=$user['masterid'];
                if($user['jltype']==3){
                    $user['masterid']=M("coach")->field('nickname')->where("userid='$masterid'")->find()['nickname'];
                }else{
                    $user['masterid']=M("school")->field('nickname')->where("userid='$masterid'")->find()['nickname'];
                }
            break;
            case 'jx':
                $field.=",fullname";
                $user=M("school")->field($field)->where("userid='$userid'")->find();
            break;
            case 'zdy':
                $field.=",teachedage,driverage";
                $user=M("guider")->field($field)->where("userid='$userid'")->find();
            break;
            default:
                $user=M("user")->field($field)->where("userid='$userid'")->find();
            break;
        }
        return $user;
    }
    function returnCoach($userid,$tp,$start,$jltype){
        $coachs=M("coach")->field("userid,nickname,img")->where("$tp='$userid' and jltype=$jltype")->limit("$start,10")->select();
        //加上小打工的教练的学员人数和订单数
        foreach ($coachs as $k=>$v){
            ...
            $coachs[$k]['train']=M("coachtrain c")->field("t.trname")->join("xueche1_train t on c.trainid=t.id and c.coachid='$uid'")->find()['trname'];
        }
        return $coachs;
    }
	function returnCoach1($userid,$tp,$start,$jltype){
        ...
        if($coachs){
            ...
            foreach ($coachs as $k=>$v){
                ...
                $trainid=rtrim(M("coachtrains")->field("trainid")->where("coachid='$uid'")->find()['trainid'],',');
            	 if($trainid){
                    $coachs[$k]['train']=M('train')->field('trname')->where("id=$trainid")->find()['trname'];
                }else{
                    $coachs[$k]['train']=null;
                }
	    }
    ...
    function returnCoachDetail($userid,$objectid,$field){
        $m = M("coach u");
        $mm = $m->field($field)->where("userid='$objectid'")->find();  
        ...
        ->join("xueche1_evaluating e on e.masterid=u.userid and e.objecthingid='$evalmasterid'")->order('e.id desc')->limit(2)->select();
        $p2 =M("evaluating")->field("id,masterid,time,masterid,content,score")->where("masterid='1' and objecthingid='$evalmasterid'")->select();
        ...
        //课程
        $trainclass=M("trainclass")->field("tcid,name,include,masterid,time,carname,mode,officialprice,whole517price,prepay517price,prepay517deposit,classtime")->where("masterid='$classmasterid'")->select();
        $mm['class'] = $trainclass;
        // 查看用户是否关注了该对象
...
/***************************************************************************************************/
	function getSchoolList1($start,$cityid,$latitude,$longitude)
	{
	    ...
	    if ($m->count()> 0) {
	        ...
	        for($k=0;$k<count($mm);$k++){
	            ...
	            $mm[$k]['student']=M('user')->field("nickname")->where("userid='$stuid'")->find()['nickname'];
	            $mm[$k]['signtime']=$stu['stime'];
	            $land[$k]=M("schoollands")->field("landmarkid")->where("schoolid='$mas'")->find()['landmarkid'];
		    if(substr_count($land[$k],',')>6){
	            ...
	            $mm[$k]['landmark']=M("landmark")->field("id,longitude,latitude")->where($where)->select();
	        }
	        $info=result(0, $mm);
	    } else {
	        $info = type(1, "没有任何驾校" . $erro, - 1);
	...
	function getCoachList1($start,$cityid,$latitude,$longitude)
	{
	    ...
	    if ($m->count() > 0) {
	        ...
	        foreach($mm as $k=>$v){
	            ...
	            $mm[$k]['location']=M('address')->field("latitude,longitude")->where("masterid='$id'")->find();
	            $mm[$k]['minprice']=M('trainclass')->field("min(whole517price)as minprice")->where("masterid='$mas'")->find()['minprice'];
	            $land[$k]=M("coachlands")->field("landmarkid")->where("coachid='$id'")->find()['landmarkid'];
		   if(substr_count($land[$k],',')>6){
	                $n = 0;
    	            for($i = 1;$i <= 5;$i++) {
                    	$n = strpos($land[$k], ',', $n);
	...
	function getGuiderList1($start,$cityid,$latitude,$longitude)
	{
	    ...
	    if ($m->count() > 0) {
	        ...
	        foreach($mm as $k=>$v){
	            ...
	            $mm[$k]['location']=M('address')->field("latitude,longitude")->where("masterid='$mas'")->find();
	            $mm[$k]['minprice']=M('trainclass')->field("min(whole517price)as minprice")->where("masterid='$mas'")->find()['minprice'];
	            $land[$k]=M("guiderlands")->field("landmarkid")->where("guiderid='$mas'")->find()['landmarkid'];
		    if(substr_count($land[$k],',')>6){
	                $n = 0;
    	            for($i = 1;$i <= 5;$i++) {
                    	$n = strpos($land[$k], ',', $n);
 ...
 function coachDetail($userid,$objectid,$field){
     ...
     ->join("xueche1_evaluating e on e.masterid=u.userid and e.objecthingid='$evalmasterid'")->order('e.id desc')->find();
     if(!$mm['message']){
         $mm['message'] = M("evaluating")->field("id,time,masterid,objectid,content,score,num,evaluate")
         ->where("objecthingid='$evalmasterid'")->order('id desc')->find();
     }
     //提问
