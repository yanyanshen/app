...
//	dump($result_arr);
    ...
    function yinlian($orderId,$respCode,$merId){
        if($respCode=='00'){
           // $post['listid']=$orderId;//------订单号
            $post['trade_no']=$_POST['queryId'];//------订单号
            $post['gmt_payment']=date('Y-m-d H:i:s');
            $post['total_fee']=$_POST['txnAmt']/100;
            $post['payment_type']=3;//---3表示银联支付
	    $post['state']=1;
	    M('list')->where("listid='$orderId'")->save($post);
            $objectid=M('list')->field("prices,masterid,objectid,type,classid,mode,total_fee")->where("listid='$orderId'")->find();
            ...
            if($objectid['prices']==($_POST['txnAmt']/100)){
                $mode=0;
            }else{
                $mode=1;
            }
		...
		$res['paidnum']=$objectid['total_fee'];
                M("reservation")->where("listid='$orderId'")->save($res);
            }
        }else{
        	foreach($_POST  as $k=>$v){
			file_put_contents('/a/c.txt',$k.'------------'.$v."\r\n",FILE_APPEND);
		}
	}
    }
}
