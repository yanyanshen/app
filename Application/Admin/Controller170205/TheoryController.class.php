...
class TheoryController extends CommonController{
    public function traffictheoryone(){
        try {
            $start=$_POST['start'];
            $mm=M('theory')->field("thnum")->limit("$start,10")->select();
            $info=result(0,$mm);
        } catch (Exception $e) {
            $info = type(1, '返回失败', - 1);
        } 
...
public function zjtestnum1($subjects=0,$jtype='C1',$address='',$userid=''){
        try {
            ...
            if($subjects==0){
                ...
                $where=$where.$w1;
                $mm=M('smallclass s')->field("count(t.id) as num,s.classname,t.ChapterId")->join("xueche1_theory t on classsmalltype=ChapterId and $where and cartype=0")->group("t.ChapterId")->select();
		...
		$privince=M('province')->field('id')->where("province like '%$address%'")->find()['id'];
                if($privince){
		...
		$data=[];
                $mm=M('smallclass4 s')->field("count(t.id) as num,s.classname,t.ChapterId")->join("xueche1_theory t on s.classsmalltype=t.ChapterId and $where ")->group("s.classname")->select();
            }
           $mm=array_merge($mm,$data);	
			foreach($mm as $k=>$v){
				$chapterid=$mm[$k]['chapterid'];
				$mm[$k]['speed']=M("speed")->field("tid,(indexs-1) as indexs")->where("userid='$userid' and subjectid=$subjects and ChapterId=$chapterid")->find();
			}
            $info=result(0,$mm);
        } catch (Exception $e) {
            $info = type(1, '返回失败', -1);
        ...
        try {
            ...
            if($subject==0){
                ...
                if(M('province')->where("province like '%$addressid%'")->count()==1){
                    $id=M('province')->field("id")->where("province like '%$addressid%'")->select()[0]['id'];
                    $data[1]['num']=M("theory")->field("count(id) as num")->where("AddressId{$id}=1")->count();
                    $data[1]['chapterid']=5;
					$data[1]['classname']=$addressid."题库";
                }
                $mm=M('smallclass s')->field("count(t.id) as num,s.classname,t.ChapterId")->join("xueche1_theory t on classsmalltype=ChapterId and $where")->group("t.ChapterId")->select();
            }else{
				$data=[];
                $mm=M('smallclass4 s')->field("count(t.id) as num,s.classname,t.ChapterId")->join("xueche1_theory t on s.classsmalltype=t.ChapterId and $where ")->group("s.classname")->select();
            }
           $mm=array_merge($mm,$data);	
			foreach($mm as $k=>$v){
				$chapterid=$mm[$k]['chapterid'];
				$mm[$k]['speed']=M("speed")->field("tid,(indexs-1) as indexs")->where("userid='$userid' and subjectid=$subject and ChapterId=$chapterid")->find();
			}
            $info=result(0,$mm);
        } catch (Exception $e) {
            $info = type(1, '返回失败', -1);
     ...
     public function truetheory($subjects=0,$jtype='C',$address='上海'){
       try {
           ...
           if($jtype=="A1" || $jtype=="A3" || $jtype=="B1"){
               ...
               $data6=M("theory")->field($field)->where("cartype=1")->order('rand()')->limit(5)->select();
           }else if($jtype=="A2" || $jtype=="B2"){
               $len1=20;$len2=37;$len3=18;$len4=25;
               $data6=M("theory")->field($field)->where("cartype=2")->order('rand()')->limit(5)->select();
           }else if($jtype=="M"){
               $len1=20;$len2=37;$len3=18;$len4=25;
               $data6=M("theory")->field($field)->where("cartype=3")->order('rand()')->limit(5)->select();
           }
           $t=array('A1','A2','A3','B1','B2','M');
           $city=M('province')->field("id")->where("province like '%$address%'")->find()['id'];
           if(in_array($jtype,$t) && $city){
               $len1=20;$len2=32;$len3=18;$len4=20;
               $w=" or AddressId{$city}=1 ";
               $data5=M('theory')->field($field)->where("$where and classId!=2")->order('rand()')->limit(5)->select();
           }elseif(!in_array($jtype,$t) && $city){
               $len1=20;$len2=35;$len3=20;$len4=20;
               $w=" or AddressId{$city}=1 ";
               $data5=M('theory')->field($field)->where("$where and classId!=2")->order('rand()')->limit(5)->select();
           }
               $data1=M('theory')->field($field)->where("ChapterID=1 and $where and classId!=2")->order('rand()')->limit($len1)->select();
               $data2=M('theory')->field($field)->where("ChapterID=2 and $where and classId!=2")->order('rand()')->limit($len2)->select();
               $data3=M('theory')->field($field)->where("ChapterID=3 and $where and classId!=2")->order('rand()')->limit($len3)->select();
               $data4=M('theory')->field($field)->where("ChapterID=4 and $where and classId!=2")->order('rand()')->limit($len4)->select();
               ...
               foreach($data as $k=>$v){
                   $id=$v['id'];
                   $data[$k]['analysis']=M('reply')->field("analysis")->where("tid=$id")->find()['analysis'];
    ...
    public function truetheory4($subjects,$jtype){
       try {
           ...
           $w=" and AddressId=1 ";
           $field="id,question,answer,a,b,c,d,times,errortimes,degree,ChapterID,ClassId,imgurl";
		   $data1=M('theory')->field($field)->where("$where and ClassId!=2")->order('rand()')->limit(45)->select();
           $data2=M('theory')->field($field)->where("$where and ClassId=2")->order('rand()')->limit(5)->select();
       ...
       try {
           ...
           if(isset($_POST['start'])){
               ...
               if(isset($_POST['nextmode'])){
                   if($_POST['nextmode']==0){
                       $where.=" and id<$start";
						$data=M('theory')->field("id,question,answer,a,b,c,d,imgurl,classid,degree,ChapterId,times,errortimes")->where($where)->order("id desc")->find();
                       $index= --$_POST['index'];
                   }elseif($_POST['nextmode']==1){
                       $where.=" and id>$start";
					   $data=M('theory')->field("id,question,answer,a,b,c,d,imgurl,classid,degree,ChapterId,times,errortimes")->where($where)->find();
                       $index=  ++$_POST['index'];
                   }
				   }else{
						$data=M('theory')->field("id,question,answer,a,b,c,d,imgurl,classid,degree,ChapterId,times,errortimes")->where($where)->find();
                   ...
                   $tid=M('speed')->field("tid,indexs")->where($wheres)->find();
                   if($tid){
                      $data=M('theory')->field("id,question,answer,a,b,c,d,imgurl,classid,degree,ChapterId,times,errortimes")->where("subjects=$subjects")->find();
					   $index=1;
                   }else{
                       $id=$tid['tid'];
                       $index=$tid['indexs'];
					    $data=M('theory')->field("id,question,answer,a,b,c,d,imgurl,classid,degree,ChapterId")->where("id>$id and where")->find();
                   }
               }
               $tid=$data['id'];
               $data['analysis']=M("reply")->field('analysis')->where("tid=$tid")->find()['analysis'];
               if($data["imgurl"]!=null){
               ...
               $num['indexs']=$index;
               $mm=M('speed')->field("tid")->where($wheres)->find()['tid'];
               if($mm){
		   ...
		   if(isset($_POST['nextmode'])){
					  if($_POST['nextmode']==0){
						   $where.="and e.id<$start";
							 $data=M('theory t')->field($field)->join("xueche1_errortheory e on e.tid=t.id and $where ")->order("e.id desc")->find();
						   --$index;
					   }elseif($_POST['nextmode']==1){
						   $where.="and e.id>$start";
						   ++$index;
							 $data=M('theory t')->field($field)->join("xueche1_errortheory e on e.tid=t.id and $where ")->find();
					   }
		   }else{
			    $data=M('theory t')->field($field)->join("xueche1_errortheory e on e.tid=t.id and $where")->find();
		   }
   ...
   public function Mytheory($subjects,$jtype,$userid){
       try {
		        ...
		        $data=M($table)->field('classname,classsmalltype')->group("classname")->select();
				   if($jtype=="A1" || $jtype=="A3" || $jtype=="B1"){
				...
				foreach($data as $k=>$v){
					...
					$data[$k]['num']=M('quebank')->field("count(id) as num,ChapterId")->where("$wheres and ChapterId=$chapterid and jtype='$jtype'")->find();
   ...
   public function savescore($userid){
       try {
           ...
           if(M('userscore')->add($_POST)){
                $data=M('userscore')->field("id,userid,score,usetime,stime")->where("userid='$userid'")->order("id desc")->limit(1)->find();
                $info=result(0,$data);
           }else{
                $info=type(2,'保存失败',-1);
           }
   ...
   public function returnscore($userid){
       try {
           $data=M('userscore')->field("id,userid,score,usetime,stime")->where("userid='$userid'")->order("id desc")->limit(10)->select();
           $info=result(0,$data);
       } catch (Exception $e) {
           $info=type(1,'返回失败',-1);
       }
       ...
       try {
           ...
           if($_POST['start']>0){
               ...
               if(isset($_POST['nextmode'])){
                   if($_POST['nextmode']==0){
                       $where.="and q.id<$start";
					    $data=M('quebank q')->field($field)->join("xueche1_theory t on q.tid=t.id and $where")->order("q.id desc")->find();
                       --$index;
                   }elseif($_POST['nextmode']==1){
                       $where.="and q.id>$start";
					   $data=M('quebank q')->field($field)->join("xueche1_theory t on q.tid=t.id and $where")->find();
                       ++$index;
                   }
			     }else{
					     $data=M('quebank q')->field($field)->join("xueche1_theory t on q.tid=t.id and $where")->find();
           ...
           if($subjects==0){
            ...
            if($subjects==0 && $chapterid==5){
                if(M('province')->where("province like '%$address%'")->count()==1){
                    $id=M('province')->field("id")->where("province='$address'")->find()['id'];
               ...
               if(M('errortheory')->where("userid='$userid' and tid=$tid")->count()==1){
				...
				if(isset($_POST['nextmode'])){
					if($_POST['nextmode']==0){
						 ...
						 $where .="and id<$start";
						 $data=M('theory')->field($field)->where($where)->order("id desc")->find();
						 $num['indexs']=$index;
						 $index--;
					}else{
						$where .="and id>$start";
							$data=M('theory')->field($field)->where($where)->find();
							 $num['indexs']=$index;
						$index++;
		  ...
		  if($chapterid!=1 && $_POST['index']==0 ){
			...
			if(isset($_POST['start'])){
              ...
              // $tid=$start;
               ...
               if(M('errortheory')->where("userid='$userid' and tid=$start")->find()){
		   ...
		   //如果不是从开始做
			if($index>0){
			 ...
			 if($_POST['flag']!=0){
		           ...
		           $_POST['ChapterId']=$chapterid;
		           $_POST['indexs']=$index;
				  if(M('speed')->where("userid='$userid' and subjectid=$subjects and ChapterId=$chapterid")->find()){
			...
			$data=M('theory')->field($field)->where($where)->order("id asc")->find();
			}else{//上一题
	...
	public function zjtestnum($subjects=0,$jtype='C1',$address='',$userid=''){
        try {
            ...
            if($subjects==0){
				...
				$privince=M('province')->field('id')->where("province like '%$address%'")->find();
                if($privince){
                    $id=$privince['id'];
                    $data[1]['num']=M("theory")->field("count(id) as num")->where("AddressId{$id}=1")->count();
                    $data[1]['chapterid']=5;
           ...
           $mm=array_merge($mm,$data);	
			foreach($mm as $k=>$v){
				$chapterid=$mm[$k]['chapterid'];
				$mm[$k]['speed']=M("speed")->field("tid,indexs")->where("userid='$userid' and subjectid=$subjects and ChapterId=$chapterid")->find();
			}
            $info=result(0,$mm);
        } catch (Exception $e) {
            $info = type(1, '返回失败', -1);
