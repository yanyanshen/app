...
class IndexController extends CommonController
{
	...
	$po=M('auth')->field("auth,time")->where("phone='$account'")->find();
	$t=time()-$po['time'];
	if ($t < 600) {
		$tureauth=$po['auth'];
            if ($tureauth== $auth) {
		    ...
		    $_POST['userid'] =guid();
                    ...
                    if ($m->add($_POST)) {
                        ...
                        $u = M('conf');
                        $_POST['downloadURL'] = $u->field("downloadURL")->find()['downloadURL'];
	...
	public function coachUinfo($type='jl')
    {
        try {
			...
			if($type=='jl'){
                ...
                ->find();
				  $user['train']=M("coachtrain c")->field("t.id,t.trname")->join("xueche1_train t on t.id=c.trainid and c.coachid='$userid'")->find();
				  if($user['jltype']==3){
					    $coachlid=$user['masterid'];
					    $user['master']=M("school")->field("userid,nickname")->where("userid='$coachlid'")->find();
						//$user['train']=M("coachtrain c")->field("t.trname")->join("xueche1_train t on t.id=c.trainid and c.coachid='$userid'")->find()['trname'];
				  }else{
					  	$schoolid=$user['masterid'];
						$user['master']=M("school")->field("userid,nickname")->where("userid='$schoolid'")->find();
				  }
			}else{
				 if($type=='jx'){
                   $field.=',fullname';
    ...
    public function myconcern($userid,$start)
    {
        try {
            ...
            foreach ($becon as $v) {
				...
				$m = M($table);
                $objectid = $v['objectid'];
                $cons[]= $m->field('userid,account,sex,nickname,signature,type,img')->where("userid='$objectid'")->find();
            }
            $info =result(0, $cons);
        } catch (Exception $e) {
            $info = type(1, "返回失败", - 1);
   ...
   public function updateuinfo()
    {
        ...
        if ($m->where("userid='$userid'")->save($_POST)) {
            ...
            if(isset($_POST['trainid'])){
                ...
                $user['trname']=M("coachtrain t")->field("r.trname")->join("xueche1_train r on t.trainid=r.id and t.coachid='$userid'")->find()['trname'];
            }
            if(isset($_POST['masterid'])){
                $schoolid=$_POST['masterid'];
				 M("coachtrain")->where("coachid='$userid'")->setField("schoolid",$schoolid);
                $user['schoolname']=M("school")->field("nickname")->where("userid='$schoolid'")->find()['nickname'];
            }
            $info = update(0, "修改成功",$user);
        } else {
    ...
    public function anversion()
    {
        try {
            $mm = M('version')->field("maxversion")
                ->where("id=1 or id=3")
                ->select();
            $v = M('conf')->field("downloadurl")->select();
	...
	$post['auth']=$cod;	
	$post['time']=time();
	if(M('auth')->where("phone='$phone'")->find()){
    ...
    public function feedback(){
     ...
     public function getUserName($userid="F5DCBE66_B741_D0EA_91ED_3A137EE99519"){
        try {
            $m=M("user");
            $name=$m->field("nickname")->where("userid='$userid'")->find();
			if($name){
			    echo result(0, $name);return;
			}else if(M("school")->field("nickname")->where("userid='$userid'")->find()){
				$name=M("school")->field("nickname")->where("userid='$userid'")->find();
				echo result(0, $name);return;
			}else if(M("coach")->field("nickname")->where("userid='$userid'")->find()){
				$name=M("coach")->field("nickname")->where("userid='$userid'")->find();
				echo result(0, $name);return;
			}else{
				$name=M("guider")->field("nickname")->where("userid='$userid'")->find();
				echo result(0, $name);return;
			}
        } catch (Exception $e) {
            $info = type(1, "返回出错");
	...
	public function userisset($userid){
		  try {
			...
			$count+=M("school")->where("userid='$userid'")->count();
			$count+=M("coach")->where("userid='$userid'")->count();
			$count+=M("guider")->where("userid='$userid'")->count();
			if($count==1){
				$info = type(0,"存在");
			}else{
				$info = type(1,"不存在");
    ...
    public function coachUinfo1($userid='E7D96389-D40B-EA8B-46DF-99FF792C0D27',$type='jl')
    {
        try {
            ...
            if($type=='jl'){
                ...
                ->find();
                $train=M('coachtrains')->field("trainid")->where("coachid='$userid'")->find()['trainid'];
                if($train){
                    $train=rtrim($train,',');
                    $user['train']=M('train')->field('id,trname')->where("id=$train")->find();
                }else{
                    $user['train']=[];
                }
                if($user['jltype']==3){
                    $coachlid=$user['masterid'];
                    $user['master']=M("coach")->field("userid,nickname")->where("userid='$coachlid'")->find();
                }else{
                    $schoolid=$user['masterid'];
                    $user['master']=M("school")->field("userid,nickname")->where("userid='$schoolid'")->find();
                }
