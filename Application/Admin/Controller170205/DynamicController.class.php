...
class DynamicController extends CommonController
{
    public function getMyDynamic($userid,$start)  
    {
        ...
        // 0代表只返回数据
        $mm=M("news")->field($field)->where("userid='$userid'")->order("id desc")->limit("$start,10")->select();
        foreach ($mm as $k=>$v){
            $id=$v['newsid'];
            $mm[$k]['imgurl']=M("newsimg")->field("imgname as imgurl")->where("imgid=$id and userid='$userid'")->select();
    ...
    public function getAllDynamic($userid='',$start)
    {
        ...
        // 0代表只返回数据
        $mm['news']=M("news")->field($field)->order("id desc")->limit("$start,10")->select();
        foreach ($mm['news'] as $k=>$v){
            $id=$v['newsid'];
            $uid=$v['masterid'];
            $table=shenfen($v['type']);
            $mm['news'][$k]['nickname']=M($table)->field("nickname")->where("userid='$uid'")->find()['nickname'];
            $mm['news'][$k]['img']=M($table)->field("img")->where("userid='$uid'")->find()['img'];
            $mm['news'][$k]['imgurl']=M("newsimg")->field("imgname as imgurl")->where("imgid=$id")->select();
    ...
    public function getNewsDetail($newsid){
        try {
            ...
            //评论
            $mm=M("news")->field($field)->where("newsid=$newsid")->find();
            $contents=M('dynamic')->field("type,id")->where("objecthingid=$newsid and (mode=3 or mode=4)")->select();
			...
			foreach($contents as $k=>$v){
			    ...
			    $mm['message'][$k]=M('dynamic s')->field($fields)->join($where)->where("s.mode=3 or s.mode=4")->find();
			}
            $praise=M('dynamic')->field("masterid,type")->where("objecthingid=$newsid and mode=1")->select();
            $tran=M('dynamic')->field("masterid,type")->where("objecthingid=$newsid and mode=2")->select();
            foreach($praise as $k=>$v){
                $uid=$v['masterid'];
                $table=shenfen($v['type']);
                $praise[$k]['nickname']=M($table)->field("nickname")->where("userid='$uid'")->find()['nickname'];
            }
            foreach($tran as $k=>$v){
                $uuid=$v['masterid'];
                $table=shenfen($v['type']);
                $tran[$k]['nickname']=M($table)->field("nickname")->where("userid='$uuid'")->find()['nickname'];
            }
            $C=count($praise);
            $T=count($tran);
            if($C>0){
    ...
    public function moreEvaluation($start,$newsid){
        try {
            ...
            $mm=M('dynamic')->field("id,masterid,rtime as time,objecthingid,content,type")->where("objecthingid=$newsid and objectid='$objectid' and mode=2")->order("e.id desc")->limit("$start,10")->select();
            foreach($mm as $k=>$v){
                $uuid=$v['masterid'];
                $table=shenfen($v['type']);
                $v['nickname']=M($table)->field("nickname")->where("userid='$uuid'")->find()['nickname'];
            }
            $info=result(0,$mm);
        } catch (Exception $e){
            $info=type(1, "返回失败", '');
	...
	public function sendDynamic()
    {
        ...
        $m = M('news');
		$userid=$_POST['userid'];
        $id=$m->field("max(id) as id")->find()['id']+1;
        ...
        if ($m->add($_POST)) {
                ...
                $mm=M("news")->field($field)->where("userid='$userid'")->order("id desc")->find();
            if(count($_FILES)>0){
                $up=new Upload();
                ...
                $imgid=$mm['newsid'];
                $mm['imgurl']=M("newsimg")->field("imgname as imgurl")->where("imgid=$imgid")->select();
    ...
    public function forwDynamic($newsid,$userid,$objectid,$type)
    {
        ...
        $m=M('news');
        $news = $m->field("content,piccount,newsid")->where("newsid=$newsid")->find();
        $id=$m->field("max(newsid) as nid")->find()['nid']+1;
        $news['userid'] =$userid;
        ...
        if ($m->add($news)) {
            ...
            $img=M('newsimg')->field("userid,imgname,imgtime")->where("imgid=$newsid")->select();
            foreach($img as $k=>$v){
                $v['userid']=$userid;
                $img[$k]['imgid']=$id; 
    ...
    public function userDetails($userid,$objectid,$start){
		try {
			$type=$_POST['type'];
			$table=shenfen($type);
			$t=M($table);
			...
			$data=$t->field("nickname,signature,img,birthday,sex")->where("userid='$objectid'")->find();
			$data['news']=M("news n")->field($field)->where("userid='$objectid'")->order("id desc")->limit("$start,10")->select();
			...
			if(empty($data['news'])){
				...
				foreach ($data['news'] as $k=>$v){
					$id=$v['newsid'];
					$data['news'][$k]['imgurl']=M("newsimg")->field("imgname as imgurl")->where("imgid=$id and userid='$objectid'")->select();
	 ...
	 public function aboutMe($userid,$start=0){ 
        try {
			...
			if($start==0){
				$return1=M("dynamic")->field($field.",content")->where($where)->order("id desc")->limit("$start,5")->select();
				$return2=M("dynamic")->field($field.",content")->where($where1)->order("id desc")->limit("$start,5")->select();
			 ...
			 if(empty($return)){
					...
					foreach($return as $k=>$v){
						...
						$table=shenfen($v['type']);
						$return[$k]['nickname']=M($table)->field("nickname")->where("userid='$uid'")->find()['nickname'];
						$return[$k]['img']=M($table)->field("img")->where("userid='$uid'")->find()['img'];
						$return[$k]['news']=M("news")->field("userid,newsid,content,time,content,evalutioncount,piccount,praisecount,transmitcount,extras,tranid")
							->where("newsid=$thingid1")->find();
						$img=M("newsimg")->field("imgname")->where("imgid=$thingid1")->select();
    ...
    public function returnEvaluate(){ 
        try {
			...
			$_POST['mode']=4;
            if(M("dynamic")->add($_POST)){
               $data=M("dynamic")->field("masterid,objectid,objecthingid,content,rtime as time,flag,dynamicid")->where("masterid='$userid' and  mode=4")->order("id desc")->find();
			   $thingid1=$data['objecthingid'];
			   $data['news']=M("news")->field("newsid,content,time,content,evalutioncount,piccount,praisecount,transmitcount,extras,tranid")->where("newsid=$thingid1")->find();
			 $img1=M("newsimg")->field("imgname")->where("imgid=$thingid1")->select();
   ...
   public function delDynamic($newsid){ 
       try {
           if(M('news')->where("newsid=$newsid")->delete() ){
			   if(M('newsimg')->where("imgid=$newsid")->find()){
					M("newsimg")->where("imgid=$newsid")->delete();
			   }
			   if(M('dynamic')->where("objecthingid=$newsid")->find()){
