                ...
                <tbody><tr style="font-weight: bold;">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($v['state']); ?>-<?php echo ($v['state']==0?'待付款':($v['state']==1?'待评价':($v['state']==2?'待确认':($v['state']==3?'已完成':'已取消')))); ?></td>
                         <td><?php echo ($v['payment_type']); ?>-<?php echo ($v['payment_type']==1?'支付宝':($v['payment_type']==2?'微信':($v['state']==3?'银联':'未支付'))); ?></td>
                        ...
                        <td><a href="<?php echo U('cl_list?cl='.$v['cl_type'].'&p='.$p.'&id='.$v['id']);?>"><?php echo ($v['cl_type']=='y'?'<l style="color:red">改为未处理</l>':'<l style="color:green">改为已处理</l>'); ?></a></td>
