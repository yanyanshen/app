...
$("document").ready(function(){
	$('#sub').click(function(){
		if($("input[name='groupname']").val()==''||$("input[name='description']").val()==''){
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 组管理 <span class="c-gray en">&gt;</span> 组列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
	<form action="<?php echo U('addgroup');?>" method='post'>
		<input type="hidden" value="<?php echo ($p); ?>" name="p"/>
		添加新的组：<input type="text" class="input-text" style="width:100px;" name="groupname"/>&nbsp;&nbsp;组描述：<input type="text" class="input-text" name="description"  style="width:700px;"/>&nbsp;&nbsp;<input type="submit" class="btn btn-danger radius" value="添加组" id='sub'/>
	</form>
	</span> <span class="r">共有数据：<strong><?php echo ($count); ?></strong> 条</span> </div>
	<table class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="11">组</th>
			</tr>
			<tr class="text-c">
				<th width="40">编号</th>
				<th width="70">组名</th>
		...
		<tbody>
		   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><tr class="text-c">
			    ...
			    <td class="td-manage"><a title="编辑" href="<?php echo U('groupuser?id='.$vv['id'].'&p='.$p);?>"><i class="Hui-iconfont">&#xe62c;</i></a>
			     <a title="删除" href="<?php echo U('delgroup?id='.$vv['id'].'&p='.$p);?>" onclick="if(confirm('确定删除该组吗?')==false)return false;" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
