	...
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
		   <?php if(is_array($coachlist)): $i = 0; $__LIST__ = $coachlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td><?php if(is_array($vv['trainname'])): $i = 0; $__LIST__ = $vv['trainname'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vvv): $mod = ($i % 2 );++$i; echo ($vvv['trname']); ?>　<?php endforeach; endif; else: echo "" ;endif; ?></td>
				<td><?php echo ($vv["phone"]); ?></td>
				<td><?php echo ($vv["attcount"]); ?>条关注</td>
				<td><?php echo ($vv["stu"]); ?></td>
				<td><?php echo ($vv["newscount"]); ?>条动态</td>
				<td><?php echo ($vv['jltype']==0?'普通教练':($vv['jltype']==1?'私人教练（个人）':($vv['jltype']==2?'私人教练（领导）':'小打工'))); ?></td>
				<td><?php echo ($vv['timeflag']==0?'不支持':'支持'); ?></td>
			    <td><?php echo ($vv["address"]); ?></td> <td><?php echo ($vv["cityname"]); ?></td><td><?php echo ($vv["carNumber"]); ?></td>
			    <td><a href="<?php echo U('School/zhengjian?userid='.$vv['userid'].'&p='.$p.'&type=jl');?>"><b>证件</b></a></td>
			    ...
			    <td class="td-manage">
			     <a title="图片管理" href="<?php echo U('School/imgmanage?userid='.$vv['userid'].'&type=jl&p='.$p);?>"><i class="Hui-iconfont">&#xe638;</i></a>
			    <a title="编辑" href="<?php echo U('jlinfo?userid='.$vv['userid'].'&p='.$p);?>"><i class="Hui-iconfont">&#xe62c;</i></a>
			    <a title="删除" href="<?php echo U('delcoach?userid='.$vv['userid'].'&p='.$p);?>" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
